 ...
 */
// 执行实际传输工作
// 1. 为数据包准备TCP协议头部信息，包括序号、源地址、目的地址、校验和等；
// 2. 调用ip_queue_xmit函数，把数据包交给IP模块
...
#define SYSCTL_FLAG_SACK	0x4
		...
		*(((__u16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) | tcb->flags);
		...
		// TCP初始化时，令send_check指向tcp_v4_send_check函数，处理校验和
		...
		// TCP初始化时，令queue_xmit指向ip_queue_xmit函数
		// 由该函数把TCP数据包交给IP协议模块
		// tcp_v4_init_sock函数初始化TCP时，将tp->specific指定为[struct tcp_func]ipv4_specific
		// 集合中的queue_xmit:发送函数指针，指向ip_queue_xmit函数
 ...
 */
// 调用tcp_transmit_skb,tcp_push_one函数发送套接字缓冲区的数据
 ...
 */
// tcp_write_xmit根据NAGLE设置发送缓冲区的数据
...
/*
 * tcp_v4_connect 调用此函数
 * Build a SYN and send it off.
 * 请求建立连接，发出标记了SYN的连接请求包
 */ 
