...
// 设置URG或PUSH标志后，tcp_push函数调用__tcp_push_pending_frames函数来发送数据包
...
new_segment:
			...
			//如果需要复制的数据长度大于分段长度：则复制长度=分段长度
			/* Try to append data to the end of skb. */
			if (copy > seglen)
				copy = seglen;

			/* Where to copy to? */
			// 根据套接字缓冲区的尺寸调整需要复制的长度 copy
