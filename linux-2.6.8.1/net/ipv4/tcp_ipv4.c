...
//创建SOCK_STREAM类型的套接字后，应用程序通过connect函数请求建立TCP连接。
//connect函数通过系统调用触发该函数tcp_v4_connect的执行
//生成有SYN标志的TCP连接请求包，并发送给对方
//是三次握手的第一步
int tcp_v4_connect(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
	struct inet_opt *inet = inet_sk(sk);
	// 变量tp指向套接字结构struct sock中的TCP选项
	struct tcp_opt *tp = tcp_sk(sk);
