...
//把自定义的IP头部信息复制到IP协议字段位置
//修正IP协议信息后，该函数把数据交给IP层发送模块
			...
			unsigned int flags)
{
	...
	//把校验和设置为CHECKSUM_NONE
	skb->ip_summed = CHECKSUM_NONE;
	//设置数据起始位置
	skb->h.raw = skb->nh.raw;
	//复制地自定义的IP头部信息 (from指向起始地址)
	//到套接字缓冲区指定的头部位置
...
{
	struct inet_opt *inet = inet_sk(sk);
	struct ipcm_cookie ipc; //用来设置控制信息的结构体
	...
	//为构建IP包头格式，在[struct ipcm_cookie]型ipc中设置控制信息 
	ipc.addr = inet->saddr;
	ipc.opt = NULL;
	...
	{
		//struct flowi 结构变量记录了查找路由表的索引信息
		...
		//获取路由表项，且通过记录路由表项的变量rt返回，并获得了控制
		//数据包发送流程的几个重要信息
