...
/* 用来管理数据包分片信息
 * sk_buff->end指针指向数据包的分片信息
 * This data is invariant across clones and lives at
 * the end of the header data, ie. at skb->end.
 */
struct skb_shared_info {
	atomic_t	dataref;//使用计数。1：释放缓冲区
...
// 套接字缓冲区，用来管理网络数据包，为发/收的网络数据提供存储区域和操作方法。
struct sk_buff {
	/* These two members must be first. */
	struct sk_buff		*next;//指向套接字缓冲区链表的下一个单元
	struct sk_buff		*prev;//指向套接字缓冲区链表的上一个单元

	struct sk_buff_head	*list;
	struct sock		*sk;//套接字缓冲区所属的套接字结构

	struct timeval		stamp;//包到达的时间
	struct net_device	*dev;//收包时，dev指向接收设备；发包时，dev指向发送设备
	struct net_device	*input_dev;
	struct net_device	*real_dev;

	//指向第四层数据包的头部：TCP, UDP, ICMP, IGMP等
	union {
		struct tcphdr	*th;
		struct udphdr	*uh;
	...
	struct	sec_path	*sp;//IPSec协议的传输跟踪信息
...
#ifdef CONFIG_NETFILTER
        unsigned long		nfmark;//网络过滤器设置信息
	__u32			nfcache;
	struct nf_ct_info	*nfct;
#ifdef CONFIG_NETFILTER_DEBUG
        unsigned int		nf_debug;//网络过滤器调试信息
#endif
#ifdef CONFIG_BRIDGE_NETFILTER
	struct nf_bridge_info	*nf_bridge;//网络过滤器桥接信息
#endif
...
// 用来初始化struct sk_buff_head结构
// 需在链表操作前调用该函数，且不可重复操作
